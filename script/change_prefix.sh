...
# Info    : Shell script to convert wordpress database prefix to another
# Version : 1.0
# changelog :
# 1.0
#   - Initial Release
...
V='1.0'
URL='https://raw.githubusercontent.com/percioandrade/wpcollection/main/script/change_prefix.sh'

# Skip version check
if [[ $1 == "-n" ]] || [[ $1 == "--noversion" ]]; then
    echo "[!] We will not check this script version"
else
    # Check script version
    V_URL=$(GET ${URL}|grep -m1 "VERSION="|cut -d '"' -f2)
    if [[ ${V} != ${V_URL} ]];then
        echo "[!] A new update for this script was released. Version $V_URLplease"
...
echo -e "[!] Please insert the new prefix. Example: dev )\n"
echo -e "[!] Don't insert long value, the recommended is 2 a 4 characters"
...
if [[ "${RESPONSE}" == "y" ]]; then
    ...
    if [[ -n "$NEW_PREFIX" ]]; then
        ...
        mysql -u "${DB_USER}" -p"${DB_PASS}" -h "${DB_HOST}" "${DATABASE}" -e "UPDATE \`${NEW_PREFIX}options\` SET option_name = '${NEW_PREFIX}user_roles' WHERE option_name = '${PREFIX}user_roles';"

        mysql -u "${DB_USER}" -p"${DB_PASS}" -h "${DB_HOST}" "${DATABASE}" -e "UPDATE \`${NEW_PREFIX}usermeta\` SET meta_key  = '${NEW_PREFIX}capabilities' WHERE meta_key = '${PREFIX}capabilities';"

        mysql -u "${DB_USER}" -p"${DB_PASS}" -h "${DB_HOST}" "${DATABASE}" -e "UPDATE \`${NEW_PREFIX}usermeta\` SET meta_key  = '${NEW_PREFIX}user_level' WHERE meta_key = '${PREFIX}user_level';"

        mysql -u "${DB_USER}" -p"${DB_PASS}" -h "${DB_HOST}" "${DATABASE}" -e "UPDATE \`${NEW_PREFIX}usermeta\` SET meta_key  = '${NEW_PREFIX}autosave_draft_ids' WHERE meta_key = '${PREFIX}autosave_draft_ids';"

        echo -e "[+] Displaying values from database\n"

        echo -e "Database ${DATABASE} new prefix is ${NEW_PREFIX}\n"

        # Show tables values
        mysql -u "${DB_USER}" -p"${DB_PASS}" -h "${DB_HOST}" "${DATABASE}" -N -e "
            SELECT * FROM ${NEW_PREFIX}options WHERE option_name = '${NEW_PREFIX}user_roles';
            SELECT * FROM ${NEW_PREFIX}usermeta WHERE meta_key = '${NEW_PREFIX}capabilities';
            SELECT * FROM ${NEW_PREFIX}usermeta WHERE meta_key = '${NEW_PREFIX}user_level';
            SELECT * FROM ${NEW_PREFIX}usermeta WHERE meta_key = '${NEW_PREFIX}autosave_draft_ids';
        "

        echo -e "\n[+] All values was updated"

    else
        echo "[!] New prefix is empty. No changes made."
    fi
